                    ...
                    @foreach($entries as $orders)
                        <h4 style="font-weight: 700">Статус заказа:
                            @if ($orders[0]->status === 'completed')
                                доставлен
                            @elseif ($orders[0]->status === 'removed' || $orders[0]->status === 'removedByClient')
                                отменен
                            @endif
                        </h4>
                        <br>
                        ...
                        @if ($orders[0]->status === 'completed' && $orders[0]->is_rate === 0)
                            <div class="text-center">
                                <button data-id="{{ $orders[0]->id }}" data-courier_id="{{ $orders[1]->id }}"
                                        type="button" class="btn btn-primary rateBtn" data-toggle="modal"
                                        data-target=".bs-example-modal-sm">Оценить курьера
                                </button>
                            </div>
                        @endif
                        @if ($orders[0]->status === 'removed' || $orders[0]->status === 'removedByClient')
                            {{ csrf_field() }}
                            <div class="text-center">
